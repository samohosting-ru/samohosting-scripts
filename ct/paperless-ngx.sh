  ...
  if [ "$UPD" == "1" ]; then
    if [[ "${RELEASE}" != "$(cat /opt/${APP}_version.txt)" ]] || [[ ! -f /opt/${APP}_version.txt ]]; then
      if [[ "$(gs --version 2>/dev/null)" != "10.04.0" ]]; then
        msg_info "Обновляю Ghostscript (Patience)"
        cd /tmp
