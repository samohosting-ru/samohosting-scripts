...
function update_script() {
  ...
  UPD=$(whiptail --backtitle "Proxmox VE Helper Scripts: Samohosting Edition v0.6.1" --title "ПОДДЕРЖКА" --radiolist --cancel-button Exit-Script "Spacebar = Select" 11 58 4 \
    "1" "Update LXC" ON \
  ...
  if [ "$UPD" == "2" ]; then
    ...
    msg_info "Устанавливаю Cockpit file sharing"
  ...
  if [ "$UPD" == "3" ]; then
    ...
    apt-get install -y samba-common-bin &>/dev/null
    msg_ok "Installed dependencies"
    msg_info "Устанавливаю Cockpit identities"
  ...
  if [ "$UPD" == "4" ]; then
    msg_info "Устанавливаю dependencies"
    apt-get install -y rsync &>/dev/null
    apt-get install -y zip &>/dev/null
    msg_ok "Installed dependencies"
    msg_info "Устанавливаю Cockpit navigator"
