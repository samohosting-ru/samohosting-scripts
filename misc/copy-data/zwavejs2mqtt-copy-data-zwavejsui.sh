...
while [ -z "${CTID_FROM:+x}" ]; do
  CTID_FROM=$(whiptail --backtitle "Proxmox VE Helper Scripts: Samohosting Edition v0.6.1" --title "$TITLE" --radiolist \
    "\nWhich Zwavejs2MQTT LXC would you like to copy FROM?\n" \
    16 $(($MSG_MAX_LENGTH + 23)) 6 \
    "${CTID_MENU[@]}" 3>&1 1>&2 2>&3) || exit
done
while [ -z "${CTID_TO:+x}" ]; do
  CTID_TO=$(whiptail --backtitle "Proxmox VE Helper Scripts: Samohosting Edition v0.6.1" --title "$TITLE" --radiolist \
...
whiptail --backtitle "Proxmox VE Helper Scripts: Samohosting Edition v0.6.1" --defaultno --title "$TITLE" --yesno \
